#!/usr/bin/env node
/**
 * Read-only FM3 CAB Room Shape hardware validation.
 *
...
const ROOMSHAPE_PARAM_ID = 56;
const ROOMSHAPE_INDEX = CHANNEL * STRIDE + ROOMSHAPE_PARAM_ID;
...
const roomShapeParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === ROOMSHAPE_PARAM_ID);
assertEqual(roomShapeParam?.name, 'CABINET_ROOMSHAPE', 'catalog param name');
assertEqual(roomShapeParam?.unit, 'enum', 'catalog unit');
const roomShapeRange = FM3_RANGES.CABINET[ROOMSHAPE_PARAM_ID];
assertEqual(roomShapeRange?.kind, 'enum', 'range kind');
assertEqual(roomShapeRange?.displayMin, 0, 'range displayMin');
assertEqual(roomShapeRange?.displayMax, 1, 'range displayMax');
assertEqual(roomShapeRange?.enumCount, 2, 'range enumCount');
assertEqual(ROOMSHAPE_INDEX, 162, 'channel-B Room Shape bulk index');
...
try {
  ...
  const raw = values[ROOMSHAPE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_ROOMSHAPE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_ROOMSHAPE paramId=${ROOMSHAPE_PARAM_ID} channel=B index=${ROOMSHAPE_INDEX}`);
