#!/usr/bin/env node
/**
 * Read-only FM3 CAB Preamp Type hardware validation.
 *
...
const PRETYPE_PARAM_ID = 43;
const PRETYPE_INDEX = CHANNEL * STRIDE + PRETYPE_PARAM_ID;
...
const preTypeParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === PRETYPE_PARAM_ID);
assertEqual(preTypeParam?.name, 'CABINET_PRETYPE', 'catalog param name');
assertEqual(preTypeParam?.unit, 'enum', 'catalog unit');
const preTypeRange = FM3_RANGES.CABINET[PRETYPE_PARAM_ID];
assertEqual(preTypeRange?.kind, 'enum', 'range kind');
assertEqual(preTypeRange?.displayMin, 0, 'range displayMin');
assertEqual(preTypeRange?.displayMax, 11, 'range displayMax');
assertEqual(preTypeRange?.enumCount, 12, 'range enumCount');
assertEqual(PRETYPE_INDEX, 149, 'channel-B Preamp Type bulk index');
...
try {
  ...
  const raw = values[PRETYPE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_PRETYPE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_PRETYPE paramId=${PRETYPE_PARAM_ID} channel=B index=${PRETYPE_INDEX}`);
