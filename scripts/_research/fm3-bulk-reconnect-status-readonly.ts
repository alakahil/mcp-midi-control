...
function openConnection(label: string) {
  ...
  const conn = connectSerial({
    ...
    log: (line) => {
      ...
      console.log(`[${label} transport] ${line}`);
    },
...
async function oneCabBulkGet(): Promise<void> {
  const { conn, path, connected } = openConnection('bulk');
  const frames: number[][] = [];

  await connected;
  console.log(`[${elapsed()}] bulk serial confirmed connected: ${path()}`);
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      const finish = (error?: Error): void => {
        ...
        try {
          ...
          console.log(`[${elapsed()}] Cab bulk GET complete: ${bulk.values.length} values`);
          resolve();
        } catch (error) {
      ...
      const unsubscribe = conn.onMessage((frame) => {
        console.log(`[${elapsed()}] bulk RX length=${frame.length}`);
    ...
    console.log(`[${elapsed()}] first serial path: ${path()}`);
  } finally {
    conn.close();
    console.log(`[${elapsed()}] first serial connection closed`);
  }
}

async function oneStatusDumpAfterReconnect(): Promise<void> {
  await new Promise<void>((resolve) => setTimeout(resolve, RECONNECT_SETTLE_MS));

  const { conn, path, connected } = openConnection('status');
  let rxCount = 0;
  ...
  const unsubscribe = conn.onMessage((frame) => {
    ...
    console.log('');
    console.log(`[${elapsed()}] status RX #${rxCount}`);
  ...
  try {
    await connected;
    console.log(`[${elapsed()}] second serial confirmed connected: ${path()}`);
    console.log(`[${elapsed()}] TX #2 STATUS_DUMP after reconnect: ${hex(statusRequest)}`);
    ...
    console.log(`RX frames after reconnect: ${rxCount}`);
...
console.log('FM3 read-only BULK -> CLOSE -> RECONNECT -> STATUS_DUMP diagnostic');

await oneCabBulkGet();
await oneStatusDumpAfterReconnect();
