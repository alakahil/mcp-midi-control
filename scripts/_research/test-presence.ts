...
const OUTPUT =
  'samples/captured/TEST_Encoded_Presence5.syx';
...
const oldPresence = amp.channels?.[scene2Channel]?.presence;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Presence = +40 bytes from channel base
const presenceOffset = channelBase + 40;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, presenceOffset, rawValue);
...
const newPresence =
  checkAmp?.channels?.[scene2Channel]?.presence;

console.log('');
console.log('FM3 Presence write test');
console.log('-----------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Presence: ${oldPresence}`);
console.log(`Requested Presence: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Presence: ${newPresence}`);
...
if (newPresence !== TARGET_VALUE) {
  throw new Error(
    `Presence verification failed: expected ${TARGET_VALUE}, got ${newPresence}`
  );
}
