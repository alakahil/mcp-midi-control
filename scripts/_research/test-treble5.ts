...
const OUTPUT =
  'samples/captured/TEST_Encoded_Treble5.syx';
...
const oldTreble = amp.channels?.[scene2Channel]?.treble;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Treble = +2 bytes from channel type word
const TrebleOffset = channelBase + 8;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, TrebleOffset, rawValue);
...
const newTreble =
  checkAmp?.channels?.[scene2Channel]?.treble;

console.log('');
console.log('FM3 Treble write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Treble: ${oldTreble}`);
console.log(`Requested Treble: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Treble: ${newTreble}`);
...
if (newTreble !== TARGET_VALUE) {
  throw new Error(
    `Treble verification failed: expected ${TARGET_VALUE}, got ${newTreble}`
  );
}
