...
const OUTPUT =
  'samples/captured/TEST_Encoded_Drive5.syx';
...
const oldDrive = amp.channels?.[scene2Channel]?.drive;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Drive = +2 bytes from channel type word
const driveOffset = channelBase + 2;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, driveOffset, rawValue);
...
const newDrive =
  checkAmp?.channels?.[scene2Channel]?.drive;

console.log('');
console.log('FM3 Drive write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Drive: ${oldDrive}`);
console.log(`Requested Drive: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Drive: ${newDrive}`);
...
if (newDrive !== TARGET_VALUE) {
  throw new Error(
    `Drive verification failed: expected ${TARGET_VALUE}, got ${newDrive}`
  );
}
