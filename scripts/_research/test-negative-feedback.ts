...
const OUTPUT =
  'samples/captured/TEST_Encoded_NegativeFeedback5.syx';
...
const oldNegativeFeedback =
  amp.channels?.[scene2Channel]?.negative_feedback;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Negative Feedback = +42 bytes from channel base
const negativeFeedbackOffset = channelBase + 42;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, negativeFeedbackOffset, rawValue);
...
const newNegativeFeedback =
  checkAmp?.channels?.[scene2Channel]?.negative_feedback;

console.log('');
console.log('FM3 Negative Feedback write test');
console.log('--------------------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Negative Feedback: ${oldNegativeFeedback}`);
console.log(`Requested Negative Feedback: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Negative Feedback: ${newNegativeFeedback}`);
...
if (newNegativeFeedback !== TARGET_VALUE) {
  throw new Error(
    `Negative Feedback verification failed: expected ${TARGET_VALUE}, got ${newNegativeFeedback}`
  );
}
