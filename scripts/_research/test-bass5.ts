...
const OUTPUT =
  'samples/captured/TEST_Encoded_Bass5.syx';
...
const oldBass = amp.channels?.[scene2Channel]?.bass;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Bass = +2 bytes from channel type word
const BassOffset = channelBase + 4;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, BassOffset, rawValue);
...
const newBass =
  checkAmp?.channels?.[scene2Channel]?.bass;

console.log('');
console.log('FM3 Bass write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Bass: ${oldBass}`);
console.log(`Requested Bass: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Bass: ${newBass}`);
...
if (newBass !== TARGET_VALUE) {
  throw new Error(
    `Bass verification failed: expected ${TARGET_VALUE}, got ${newBass}`
  );
}
