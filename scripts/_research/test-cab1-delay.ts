...
const OUTPUT = 'samples/captured/TEST_Encoded_Cab1Delay0.250ms.syx';
...
function cab1DelayToRaw(valueMs: number): number {
...
const delayOffset = channelBase + 0x14;
const rawValue = cab1DelayToRaw(TARGET_VALUE);
...
console.log('');
console.log('FM3 Cab 1 Delay write test');
console.log('--------------------------');
...
if (newRaw !== rawValue) {
  throw new Error(`Cab 1 Delay verification failed: expected ${rawValue}, got ${newRaw}`);
}
