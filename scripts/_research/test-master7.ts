...
const OUTPUT =
  'samples/captured/TEST_Encoded_Master7.syx';

const TARGET_VALUE = 7.00;
...
const oldMaster = amp.channels?.[scene2Channel]?.master_volume;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Master Volume = +2 bytes from channel type word
const MasterOffset = channelBase + 10;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, MasterOffset, rawValue);
...
const newMaster =
  checkAmp?.channels?.[scene2Channel]?.master_volume;

console.log('');
console.log('FM3 Master Volume write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Master Volume: ${oldMaster}`);
console.log(`Requested Master Volume: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Master Volume: ${newMaster}`);
...
if (newMaster !== TARGET_VALUE) {
  throw new Error(
    `Master Volume verification failed: expected ${TARGET_VALUE}, got ${newMaster}`
  );
}
