...
const OUTPUT = 'samples/captured/TEST_Encoded_S2_Amp_Bypassed.syx';
...
const oldBypass = amp.scene_bypass?.[1];

const newBody = decodedRaw.body.slice();

// Scene 2 channel-state = amp.offset
// A=0, B=1, C=2, D=3
writeU16LE(newBody, amp.offset + 16, 1);
...
const newBypass = checkAmp?.scene_bypass?.[1];

console.log('');
console.log('FM3 Scene 2 Amp channel write test');
console.log('----------------------------------');
console.log(`Old bypass: ${oldBypass}`);
console.log(`Requested bypass: true`);
console.log(`Re-decoded bypass: ${newBypass}`);
...
if (newBypass !== true) {
  throw new Error(`Bypass verification failed: expected true, got ${newBypass}`);
}
