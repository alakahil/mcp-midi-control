...
const OUTPUT =
  'samples/captured/TEST_Encoded_Depth5.syx';
...
const oldDepth = amp.channels?.[scene2Channel]?.depth;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Depth = +32 bytes from channel base
const depthOffset = channelBase + 32;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, depthOffset, rawValue);
...
const newDepth =
  checkAmp?.channels?.[scene2Channel]?.depth;

console.log('');
console.log('FM3 Depth write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Depth: ${oldDepth}`);
console.log(`Requested Depth: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Depth: ${newDepth}`);
...
if (newDepth !== TARGET_VALUE) {
  throw new Error(
    `Depth verification failed: expected ${TARGET_VALUE}, got ${newDepth}`
  );
}
