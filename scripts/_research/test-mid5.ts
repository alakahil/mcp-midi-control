...
const OUTPUT =
  'samples/captured/TEST_Encoded_Mid5.syx';
...
const oldMid = amp.channels?.[scene2Channel]?.mid;

const channelBase =
  amp.params_offset + ch * amp.cols * 2;

// FM3 Amp Mid = +2 bytes from channel type word
const MidOffset = channelBase + 6;

const rawValue = knob10ToRaw(TARGET_VALUE);

const newBody = decodedRaw.body.slice();

writeU16LE(newBody, MidOffset, rawValue);
...
const newMid =
  checkAmp?.channels?.[scene2Channel]?.mid;

console.log('');
console.log('FM3 Mid write test');
console.log('--------------------');
console.log(`Preset: ${check.preset_name}`);
console.log(`Scene 2 Amp Channel: ${scene2Channel}`);
console.log(`Old Mid: ${oldMid}`);
console.log(`Requested Mid: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Mid: ${newMid}`);
...
if (newMid !== TARGET_VALUE) {
  throw new Error(
    `Mid verification failed: expected ${TARGET_VALUE}, got ${newMid}`
  );
}
