...
const OUTPUT =
  'samples/captured/TEST_Encoded_CabLevel2.syx';

const TARGET_VALUE = 2.00;
...
function cabLevelToRaw(valueDb: number): number {
  return Math.floor(((valueDb + 80) / 100) * 65535);
}
...
const levelOffset = channelBase + 44;

const rawValue = cabLevelToRaw(TARGET_VALUE);
...
const decodeCabLevel = (raw: number) =>
  (raw / 65535) * 100 - 80;
...
console.log(`Old decoded Level: ${decodeCabLevel(oldRawLevel)}`);
console.log(`Requested Level: ${TARGET_VALUE.toFixed(2)}`);
console.log(`Re-decoded Level: ${decodeCabLevel(newRawLevel)}`);
