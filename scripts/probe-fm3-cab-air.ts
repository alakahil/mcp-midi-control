#!/usr/bin/env node
/**
 * Read-only FM3 CAB Air hardware validation.
 *
...
const AIR_PARAM_ID = 54;
const AIR_INDEX = CHANNEL * STRIDE + AIR_PARAM_ID;
...
const airParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === AIR_PARAM_ID);
assertEqual(airParam?.name, 'CABINET_AIR', 'catalog param name');
assertEqual(airParam?.unit, 'percent', 'catalog unit');
assertEqual(airParam?.displayMin, 0, 'catalog displayMin');
assertEqual(airParam?.displayMax, 100, 'catalog displayMax');
assertEqual(AIR_INDEX, 160, 'channel-B Air bulk index');
...
try {
  ...
  const raw = values[AIR_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_AIR raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_AIR paramId=${AIR_PARAM_ID} channel=B index=${AIR_INDEX}`);
