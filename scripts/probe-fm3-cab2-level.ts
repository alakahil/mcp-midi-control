#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 Level hardware validation.
 *
...
const LEVEL2_PARAM_ID = 9;
const LEVEL2_INDEX = CHANNEL * STRIDE + LEVEL2_PARAM_ID;
...
const levelDb = (raw: number): number => -40 + (raw / 65534) * 40;
...
const levelParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === LEVEL2_PARAM_ID);
assertEqual(levelParam?.name, 'CABINET_LEVEL2', 'catalog param name');
assertEqual(levelParam?.unit, 'db', 'catalog unit');
assertEqual(LEVEL2_INDEX, 115, 'channel-B LEVEL2 bulk index');
...
try {
  ...
  const raw = values[LEVEL2_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LEVEL2 raw out of range: ${String(raw)}`);
  }

  const db = levelDb(raw);
  if (db < -40.01 || db > 0.01) {
    throw new Error(`LEVEL1 display out of range: ${db.toFixed(2)} dB`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_LEVEL2 paramId=${LEVEL2_PARAM_ID} channel=B index=${LEVEL2_INDEX}`);
  console.log(`raw=${raw} display=${db.toFixed(2)} dB values=${values.length}`);
} finally {
  conn.close();
}
