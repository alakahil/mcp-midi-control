#!/usr/bin/env node
/**
 * Read-only FM3 CAB Mid hardware validation.
 *
...
const MID_PARAM_ID = 45;
const MID_INDEX = CHANNEL * STRIDE + MID_PARAM_ID;
...
const midParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === MID_PARAM_ID);
assertEqual(midParam?.name, 'CABINET_MID', 'catalog param name');
assertEqual(midParam?.unit, 'db', 'catalog unit');
const midRange = FM3_RANGES.CABINET[MID_PARAM_ID];
assertEqual(midRange?.kind, 'float', 'range kind');
assertEqual(midRange?.displayMin, -12, 'range displayMin');
assertEqual(midRange?.displayMax, 12, 'range displayMax');
assertEqual(MID_INDEX, 151, 'channel-B Mid bulk index');
...
try {
  ...
  const raw = values[MID_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_MID raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_MID paramId=${MID_PARAM_ID} channel=B index=${MID_INDEX}`);
