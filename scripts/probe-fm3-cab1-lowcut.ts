#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 Low Cut hardware validation.
 *
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LOCUT1 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_LOCUT1 paramId=${LOWCUT_PARAM_ID} channel=B index=${LOWCUT_INDEX}`);
