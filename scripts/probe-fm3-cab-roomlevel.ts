#!/usr/bin/env node
/**
 * Read-only FM3 CAB Room Level hardware validation.
 *
...
const ROOMMIX_PARAM_ID = 35;
const ROOMMIX_INDEX = CHANNEL * STRIDE + ROOMMIX_PARAM_ID;
...
const roomMixParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === ROOMMIX_PARAM_ID);
assertEqual(roomMixParam?.name, 'CABINET_ROOMMIX', 'catalog param name');
assertEqual(roomMixParam?.unit, 'percent', 'catalog unit');
assertEqual(roomMixParam?.displayMin, 0, 'catalog displayMin');
assertEqual(roomMixParam?.displayMax, 100, 'catalog displayMax');
assertEqual(ROOMMIX_INDEX, 141, 'channel-B Room Level bulk index');
...
try {
  ...
  const raw = values[ROOMMIX_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_ROOMMIX raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_ROOMMIX paramId=${ROOMMIX_PARAM_ID} channel=B index=${ROOMMIX_INDEX}`);
