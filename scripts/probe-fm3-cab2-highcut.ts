#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 High Cut hardware validation.
 *
...
assertEqual(highCutParam?.name, 'CABINET_HICUT2', 'catalog param name');
assertEqual(highCutParam?.unit, 'numeric', 'catalog unit');
assertEqual(HIGHCUT_INDEX, 173, 'channel-B High Cut bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_HICUT2 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_HICUT2 paramId=${HIGHCUT_PARAM_ID} channel=B index=${HIGHCUT_INDEX}`);
