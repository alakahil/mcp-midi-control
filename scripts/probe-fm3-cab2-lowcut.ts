#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 Low Cut hardware validation.
 *
...
assertEqual(lowCutParam?.name, 'CABINET_LOCUT2', 'catalog param name');
assertEqual(lowCutParam?.unit, 'numeric', 'catalog unit');
assertEqual(LOWCUT_INDEX, 169, 'channel-B Low Cut bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LOCUT2 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_LOCUT2 paramId=${LOWCUT_PARAM_ID} channel=B index=${LOWCUT_INDEX}`);
