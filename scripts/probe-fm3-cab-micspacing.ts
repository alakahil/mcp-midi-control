#!/usr/bin/env node
/**
 * Read-only FM3 CAB Mic Spacing hardware validation.
 *
...
const MICSPACING_PARAM_ID = 37;
const MICSPACING_INDEX = CHANNEL * STRIDE + MICSPACING_PARAM_ID;
...
const micSpacingParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === MICSPACING_PARAM_ID);
assertEqual(micSpacingParam?.name, 'CABINET_MICSPACE', 'catalog param name');
assertEqual(micSpacingParam?.unit, 'cm', 'catalog unit');
assertEqual(micSpacingParam?.displayMin, 0, 'catalog displayMin');
assertEqual(micSpacingParam?.displayMax, 100, 'catalog displayMax');
assertEqual(MICSPACING_INDEX, 143, 'channel-B Mic Spacing bulk index');
...
try {
  ...
  const raw = values[MICSPACING_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_MICSPACE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_MICSPACE paramId=${MICSPACING_PARAM_ID} channel=B index=${MICSPACING_INDEX}`);
