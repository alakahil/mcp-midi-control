#!/usr/bin/env node
/**
 * Read-only FM3 CAB Air Frequency hardware validation.
 *
...
const AIRFREQ_PARAM_ID = 55;
const AIRFREQ_INDEX = CHANNEL * STRIDE + AIRFREQ_PARAM_ID;
...
const airFreqParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === AIRFREQ_PARAM_ID);
assertEqual(airFreqParam?.name, 'CABINET_AIRFREQ', 'catalog param name');
assertEqual(airFreqParam?.unit, 'hz', 'catalog unit');
assertEqual(airFreqParam?.displayMin, 2000, 'catalog displayMin');
assertEqual(airFreqParam?.displayMax, 20000, 'catalog displayMax');
assertEqual(airFreqParam?.scaling, 'log10', 'catalog scaling');
assertEqual(AIRFREQ_INDEX, 161, 'channel-B Air Frequency bulk index');
...
try {
  ...
  const raw = values[AIRFREQ_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_AIRFREQ raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_AIRFREQ paramId=${AIRFREQ_PARAM_ID} channel=B index=${AIRFREQ_INDEX}`);
  console.log(`raw=${raw} values=${values.length}`);
  console.log(`VECTOR=${values.join(',')}`);
} finally {
  conn.close();
}
