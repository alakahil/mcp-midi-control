#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 Distance hardware validation.
 *
...
assertEqual(distanceParam?.name, 'CABINET_DYNACAB_Z2', 'catalog param name');
...
assertEqual(DISTANCE_INDEX, 204, 'channel-B Distance bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_Z2 raw out of range: ${String(raw)}`);
  }
  ...
  console.log(`CABINET_DYNACAB_Z2 paramId=${DISTANCE_PARAM_ID} channel=B index=${DISTANCE_INDEX}`);
