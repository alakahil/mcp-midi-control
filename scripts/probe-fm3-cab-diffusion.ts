#!/usr/bin/env node
/**
 * Read-only FM3 CAB Room Diffusion hardware validation.
 *
...
const DIFFUSION_PARAM_ID = 59;
const DIFFUSION_INDEX = CHANNEL * STRIDE + DIFFUSION_PARAM_ID;
...
const diffusionParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === DIFFUSION_PARAM_ID);
assertEqual(diffusionParam?.name, 'CABINET_DIFFUSION', 'catalog param name');
assertEqual(diffusionParam?.unit, 'percent', 'catalog unit');
const diffusionRange = FM3_RANGES.CABINET[DIFFUSION_PARAM_ID];
assertEqual(diffusionRange?.kind, 'float', 'range kind');
assertEqual(diffusionRange?.displayMin, 0, 'range displayMin');
assertEqual(diffusionRange?.displayMax, 100, 'range displayMax');
assertEqual(DIFFUSION_INDEX, 165, 'channel-B Room Diffusion bulk index');
...
try {
  ...
  const raw = values[DIFFUSION_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DIFFUSION raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DIFFUSION paramId=${DIFFUSION_PARAM_ID} channel=B index=${DIFFUSION_INDEX}`);
