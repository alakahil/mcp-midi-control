#!/usr/bin/env node
/**
 * Read-only FM3 CAB HF Damping hardware validation.
 *
...
const HFDAMPING_PARAM_ID = 58;
const HFDAMPING_INDEX = CHANNEL * STRIDE + HFDAMPING_PARAM_ID;
...
const hfDampingParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === HFDAMPING_PARAM_ID);
assertEqual(hfDampingParam?.name, 'CABINET_HFDAMPING', 'catalog param name');
assertEqual(hfDampingParam?.unit, 'numeric', 'catalog unit');
const hfDampingRange = FM3_RANGES.CABINET[HFDAMPING_PARAM_ID];
assertEqual(hfDampingRange?.kind, 'float', 'range kind');
assertEqual(hfDampingRange?.displayMin, 0, 'range displayMin');
assertEqual(hfDampingRange?.displayMax, 10, 'range displayMax');
assertEqual(HFDAMPING_INDEX, 164, 'channel-B HF Damping bulk index');
...
try {
  ...
  const raw = values[HFDAMPING_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_HFDAMPING raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_HFDAMPING paramId=${HFDAMPING_PARAM_ID} channel=B index=${HFDAMPING_INDEX}`);
