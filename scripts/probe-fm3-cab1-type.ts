#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 DynaCab Type hardware validation.
 *
...
assertEqual(typeParam?.name, 'CABINET_DYNACAB_TYPE1', 'catalog param name');
...
assertEqual(TYPE_INDEX, 191, 'channel-B DynaCab Type bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_TYPE1 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_TYPE1 paramId=${TYPE_PARAM_ID} channel=B index=${TYPE_INDEX}`);
