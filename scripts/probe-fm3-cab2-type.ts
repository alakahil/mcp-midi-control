#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 DynaCab Type hardware validation.
 *
...
const TYPE_PARAM_ID = 86;
const TYPE_INDEX = CHANNEL * STRIDE + TYPE_PARAM_ID;
...
const typeParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === TYPE_PARAM_ID);
assertEqual(typeParam?.name, 'CABINET_DYNACAB_TYPE2', 'catalog param name');
assertEqual(typeParam?.unit, 'count', 'catalog unit');
assertEqual(typeParam?.displayMin, 0, 'catalog displayMin');
assertEqual(typeParam?.displayMax, 31, 'catalog displayMax');
assertEqual(TYPE_INDEX, 192, 'channel-B DynaCab Type bulk index');
...
try {
  ...
  const raw = values[TYPE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_TYPE2 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_TYPE2 paramId=${TYPE_PARAM_ID} channel=B index=${TYPE_INDEX}`);
