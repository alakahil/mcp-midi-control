#!/usr/bin/env node
/**
 * Read-only FM3 CAB Floor Reflections hardware validation.
 *
...
const FLOORLVL_PARAM_ID = 53;
const FLOORLVL_INDEX = CHANNEL * STRIDE + FLOORLVL_PARAM_ID;
...
const floorLvlParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === FLOORLVL_PARAM_ID);
assertEqual(floorLvlParam?.name, 'CABINET_FLOORLVL', 'catalog param name');
assertEqual(floorLvlParam?.unit, 'bipolar_percent', 'catalog unit');
const floorLvlRange = FM3_RANGES.CABINET?.[FLOORLVL_PARAM_ID];
assertEqual(floorLvlRange?.kind, 'float', 'range kind');
assertEqual(floorLvlRange?.displayMin, -100, 'range displayMin');
assertEqual(floorLvlRange?.displayMax, 100, 'range displayMax');
assertEqual(FLOORLVL_INDEX, 159, 'channel-B Floor Reflections bulk index');
...
try {
  ...
  const raw = values[FLOORLVL_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_FLOORLVL raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_FLOORLVL paramId=${FLOORLVL_PARAM_ID} channel=B index=${FLOORLVL_INDEX}`);
