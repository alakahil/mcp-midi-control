#!/usr/bin/env node
/**
 * Read-only FM3 CAB Room Size hardware validation.
 *
...
const ROOMSIZE_PARAM_ID = 36;
const ROOMSIZE_INDEX = CHANNEL * STRIDE + ROOMSIZE_PARAM_ID;
...
const roomSizeParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === ROOMSIZE_PARAM_ID);
assertEqual(roomSizeParam?.name, 'CABINET_ROOMSIZE', 'catalog param name');
assertEqual(roomSizeParam?.unit, 'numeric', 'catalog unit');
assertEqual(roomSizeParam?.displayMin, 3, 'catalog displayMin');
assertEqual(roomSizeParam?.displayMax, 30, 'catalog displayMax');
assertEqual(ROOMSIZE_INDEX, 142, 'channel-B Room Size bulk index');
...
try {
  ...
  const raw = values[ROOMSIZE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_ROOMSIZE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_ROOMSIZE paramId=${ROOMSIZE_PARAM_ID} channel=B index=${ROOMSIZE_INDEX}`);
