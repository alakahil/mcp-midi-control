#!/usr/bin/env node
/**
 * Read-only FM3 CAB Preamp Mode hardware validation.
 *
...
const OVERSAMPLE_PARAM_ID = 47;
const OVERSAMPLE_INDEX = CHANNEL * STRIDE + OVERSAMPLE_PARAM_ID;
...
const oversampleParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === OVERSAMPLE_PARAM_ID);
assertEqual(oversampleParam?.name, 'CABINET_OVERSAMPLE', 'catalog param name');
assertEqual(oversampleParam?.unit, 'enum', 'catalog unit');
const oversampleRange = FM3_RANGES.CABINET[OVERSAMPLE_PARAM_ID];
assertEqual(oversampleRange?.kind, 'enum', 'range kind');
assertEqual(oversampleRange?.displayMin, 0, 'range displayMin');
assertEqual(oversampleRange?.displayMax, 1, 'range displayMax');
assertEqual(oversampleRange?.enumCount, 2, 'range enumCount');
assertEqual(OVERSAMPLE_INDEX, 153, 'channel-B Preamp Mode bulk index');
...
try {
  ...
  const raw = values[OVERSAMPLE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_OVERSAMPLE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_OVERSAMPLE paramId=${OVERSAMPLE_PARAM_ID} channel=B index=${OVERSAMPLE_INDEX}`);
