#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 Level hardware validation.
 *
...
const LEVEL1_PARAM_ID = 8;
const LEVEL1_INDEX = CHANNEL * STRIDE + LEVEL1_PARAM_ID;
...
const levelParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === LEVEL1_PARAM_ID);
assertEqual(levelParam?.name, 'CABINET_LEVEL1', 'catalog param name');
assertEqual(levelParam?.unit, 'db', 'catalog unit');
assertEqual(LEVEL1_INDEX, 114, 'channel-B LEVEL1 bulk index');
...
try {
  ...
  const raw = values[LEVEL1_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LEVEL1 raw out of range: ${String(raw)}`);
  }
  ...
  console.log(`CABINET_LEVEL1 paramId=${LEVEL1_PARAM_ID} channel=B index=${LEVEL1_INDEX}`);
