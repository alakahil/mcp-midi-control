#!/usr/bin/env node
/**
 * Read-only FM3 CAB Treble hardware validation.
 *
...
const TREBLE_PARAM_ID = 46;
const TREBLE_INDEX = CHANNEL * STRIDE + TREBLE_PARAM_ID;
...
const trebleParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === TREBLE_PARAM_ID);
assertEqual(trebleParam?.name, 'CABINET_TREBLE', 'catalog param name');
assertEqual(trebleParam?.unit, 'db', 'catalog unit');
const trebleRange = FM3_RANGES.CABINET[TREBLE_PARAM_ID];
assertEqual(trebleRange?.kind, 'float', 'range kind');
assertEqual(trebleRange?.displayMin, -12, 'range displayMin');
assertEqual(trebleRange?.displayMax, 12, 'range displayMax');
assertEqual(TREBLE_INDEX, 152, 'channel-B Treble bulk index');
...
try {
  ...
  const raw = values[TREBLE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_TREBLE raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_TREBLE paramId=${TREBLE_PARAM_ID} channel=B index=${TREBLE_INDEX}`);
