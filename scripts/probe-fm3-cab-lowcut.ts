#!/usr/bin/env node
/**
 * Read-only FM3 CAB Low Cut hardware validation.
 *
...
const LOWCUT_PARAM_ID = 38;
const LOWCUT_INDEX = CHANNEL * STRIDE + LOWCUT_PARAM_ID;
...
assertEqual(lowCutParam?.name, 'CABINET_LOCUT', 'catalog param name');
assertEqual(lowCutParam?.unit, 'hz', 'catalog unit');
assertEqual(lowCutParam?.displayMin, 20, 'catalog displayMin');
assertEqual(lowCutParam?.displayMax, 2000, 'catalog displayMax');
assertEqual(lowCutParam?.scaling, 'log10', 'catalog scaling');
assertEqual(LOWCUT_INDEX, 144, 'channel-B Low Cut bulk index');
...
try {
  ...
  const raw = values[LOWCUT_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LOCUT raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_LOCUT paramId=${LOWCUT_PARAM_ID} channel=B index=${LOWCUT_INDEX}`);
