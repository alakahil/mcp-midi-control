#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 DynaCab Mic hardware validation.
 *
...
assertEqual(micParam?.name, 'CABINET_DYNACAB_MIC2', 'catalog param name');
...
assertEqual(MIC_INDEX, 196, 'channel-B DynaCab Mic bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 31) {
    throw new Error(`channel-B CABINET_DYNACAB_MIC2 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_MIC2 paramId=${MIC_PARAM_ID} channel=B index=${MIC_INDEX}`);
