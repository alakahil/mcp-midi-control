#!/usr/bin/env node
/**
 * Read-only FM3 CAB High Cut hardware validation.
 *
...
assertEqual(highCutParam?.name, 'CABINET_HICUT', 'catalog param name');
assertEqual(highCutParam?.unit, 'hz', 'catalog unit');
assertEqual(highCutParam?.displayMin, 200, 'catalog displayMin');
assertEqual(highCutParam?.displayMax, 20000, 'catalog displayMax');
assertEqual(highCutParam?.scaling, 'log10', 'catalog scaling');
assertEqual(HIGHCUT_INDEX, 145, 'channel-B High Cut bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_HICUT raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_HICUT paramId=${HIGHCUT_PARAM_ID} channel=B index=${HIGHCUT_INDEX}`);
