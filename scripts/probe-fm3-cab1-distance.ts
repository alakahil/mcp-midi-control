#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 Distance hardware validation.
 *
...
const DISTANCE_PARAM_ID = 97;
const DISTANCE_INDEX = CHANNEL * STRIDE + DISTANCE_PARAM_ID;
...
const distanceCm = (raw: number): number => (raw / 65534) * 24;
...
const distanceParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === DISTANCE_PARAM_ID);
assertEqual(distanceParam?.name, 'CABINET_DYNACAB_Z1', 'catalog param name');
assertEqual(distanceParam?.unit, 'cm', 'catalog unit');
assertEqual(distanceParam?.displayMin, 0, 'catalog displayMin');
assertEqual(distanceParam?.displayMax, 24, 'catalog displayMax');
assertEqual(DISTANCE_INDEX, 203, 'channel-B Distance bulk index');
...
try {
  ...
  const raw = values[DISTANCE_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_Z1 raw out of range: ${String(raw)}`);
  }

  const cm = distanceCm(raw);
  if (cm < -0.01 || cm > 24.01) {
    throw new Error(`Distance cm out of range: ${cm.toFixed(2)} cm`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_Z1 paramId=${DISTANCE_PARAM_ID} channel=B index=${DISTANCE_INDEX}`);
  console.log(`raw=${raw} catalogCm=${cm.toFixed(2)} values=${values.length}`);
} finally {
  conn.close();
}
