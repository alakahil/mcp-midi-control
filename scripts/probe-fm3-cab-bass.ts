#!/usr/bin/env node
/**
 * Read-only FM3 CAB Bass hardware validation.
 *
...
const BASS_PARAM_ID = 44;
const BASS_INDEX = CHANNEL * STRIDE + BASS_PARAM_ID;
...
const bassParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === BASS_PARAM_ID);
assertEqual(bassParam?.name, 'CABINET_BASS', 'catalog param name');
assertEqual(bassParam?.unit, 'db', 'catalog unit');
const bassRange = FM3_RANGES.CABINET[BASS_PARAM_ID];
assertEqual(bassRange?.kind, 'float', 'range kind');
assertEqual(bassRange?.displayMin, -12, 'range displayMin');
assertEqual(bassRange?.displayMax, 12, 'range displayMax');
assertEqual(BASS_INDEX, 150, 'channel-B Bass bulk index');
...
try {
  ...
  const raw = values[BASS_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_BASS raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_BASS paramId=${BASS_PARAM_ID} channel=B index=${BASS_INDEX}`);
