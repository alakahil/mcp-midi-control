#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 Pan hardware validation.
 *
...
assertEqual(panParam?.name, 'CABINET_PAN2', 'catalog param name');
...
assertEqual(PAN_INDEX, 119, 'channel-B Pan bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_PAN2 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_PAN2 paramId=${PAN_PARAM_ID} channel=B index=${PAN_INDEX}`);
