#!/usr/bin/env node
/**
 * Controlled FM3 CAB 2 Pan hardware validation.
 *
 * Fixed outbound device operations only: Cab bulk GET, STATUS_DUMP preflight,
 * PAN2 SET to 25%, Cab bulk GET, STATUS_DUMP restore preflight, PAN2 restore
...
const BASELINE_RAW = 32767;
const TEMPORARY_RAW = 40959;
...
assertEqual(
  hex(temporarySet),
  'F0 00 01 74 11 01 52 00 3E 00 0D 00 40 00 00 79 03 00 00 00 00 4E F7',
  'temporary SET wire invariant',
);
assertEqual(
  hex(restoreSet),
  'F0 00 01 74 11 01 52 00 3E 00 0D 00 00 00 00 78 03 00 00 00 00 0F F7',
...
for (const [label, frame, raw] of [
  ...
  assertEqual(parsed.paramId, PAN2_PARAM_ID, `${label} SET paramId`);
...
async function sendPan2Set(label: 'temporary SET' | 'restore SET', frame: number[], expectedRaw: number): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    const unsubscribe = conn.onMessage((response) => {
      ...
      try {
        ...
        if (echo.effectId !== CAB_EFFECT_ID || echo.paramId !== PAN2_PARAM_ID) return;
  ...
  console.log(`${label} ACK: CABINET_PAN2 paramId 13 raw ${expectedRaw} (${panDisplay(expectedRaw)})`);
}
...
console.log('FM3 controlled CAB 2 Pan validation; prerequisite: Cab block active channel must be B.');
console.log(`serial path: ${detectedPath}; baud: ${BAUD_RATE}`);
try {
  baseline = await bulkGet('1. baseline Cab bulk GET');
  assertEqual(baseline[PAN2_INDEX], BASELINE_RAW, 'baseline channel-B PAN2 raw');
  assertPanDisplay(baseline[PAN2_INDEX], 0.0, 'baseline channel-B PAN2 display');
  ...
  await sendPan2Set('temporary SET', temporarySet, TEMPORARY_RAW);

  temporary = await bulkGet('3. temporary-state Cab bulk GET');
  assertEqual(temporary[PAN2_INDEX], TEMPORARY_RAW, 'temporary channel-B PAN2 raw');
  assertPanDisplay(temporary[PAN2_INDEX], 25.0, 'temporary channel-B PAN2 display');
  assertEqual(temporary[PAN1_INDEX], baseline[PAN1_INDEX], 'temporary channel-B PAN1 unchanged');
  ...
  if (changed.length !== 1 || changed[0] !== PAN2_INDEX) {
    throw new Error(`unrelated bulk change(s): expected only index ${PAN2_INDEX}, got [${changed.join(', ')}]`);
  }
} catch (error) {
  ...
  if (temporarySetSent && !restoreSetSent) {
    ...
    try {
      ...
      restoreSetSent = true;
      await sendPan2Set('restore SET', restoreSet, BASELINE_RAW);
    } catch (restoreError) {
...
if (temporarySetSent) {
  try {
    ...
    assertEqual(final[PAN2_INDEX], BASELINE_RAW, 'final channel-B PAN2 raw');
    assertPanDisplay(final[PAN2_INDEX], 0.0, 'final channel-B PAN2 display');
    assertEqual(final[PAN1_INDEX], baseline[PAN1_INDEX], 'final channel-B PAN1 unchanged');
