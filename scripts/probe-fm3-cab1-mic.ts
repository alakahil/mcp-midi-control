#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 DynaCab Mic hardware validation.
 *
...
const MIC_PARAM_ID = 89;
const MIC_INDEX = CHANNEL * STRIDE + MIC_PARAM_ID;
...
const micParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === MIC_PARAM_ID);
assertEqual(micParam?.name, 'CABINET_DYNACAB_MIC1', 'catalog param name');
assertEqual(micParam?.unit, 'count', 'catalog unit');
assertEqual(micParam?.displayMin, 0, 'catalog displayMin');
assertEqual(micParam?.displayMax, 31, 'catalog displayMax');
assertEqual(MIC_INDEX, 195, 'channel-B DynaCab Mic bulk index');
...
try {
  ...
  const raw = values[MIC_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 31) {
    throw new Error(`channel-B CABINET_DYNACAB_MIC1 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_MIC1 paramId=${MIC_PARAM_ID} channel=B index=${MIC_INDEX}`);
