#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 Position hardware validation.
 *
...
assertEqual(positionParam?.name, 'CABINET_DYNACAB_R1', 'catalog param name');
...
assertEqual(POSITION_INDEX, 199, 'channel-B Position bulk index');
...
try {
  ...
  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_R1 raw out of range: ${String(raw)}`);
  }
  ...
  console.log(`CABINET_DYNACAB_R1 paramId=${POSITION_PARAM_ID} channel=B index=${POSITION_INDEX}`);
