#!/usr/bin/env node
/**
 * Read-only FM3 CAB 1 Pan hardware validation.
 *
...
const PAN_PARAM_ID = 12;
const PAN_INDEX = CHANNEL * STRIDE + PAN_PARAM_ID;
...
const panParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === PAN_PARAM_ID);
assertEqual(panParam?.name, 'CABINET_PAN1', 'catalog param name');
assertEqual(panParam?.unit, 'bipolar_percent', 'catalog unit');
assertEqual(panParam?.displayMin, -100, 'catalog displayMin');
assertEqual(panParam?.displayMax, 100, 'catalog displayMax');
assertEqual(PAN_INDEX, 118, 'channel-B Pan bulk index');
...
try {
  ...
  const raw = values[PAN_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_PAN1 raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_PAN1 paramId=${PAN_PARAM_ID} channel=B index=${PAN_INDEX}`);
