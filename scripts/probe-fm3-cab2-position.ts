#!/usr/bin/env node
/**
 * Read-only FM3 CAB 2 Position hardware validation.
 *
...
const POSITION_PARAM_ID = 94;
const POSITION_INDEX = CHANNEL * STRIDE + POSITION_PARAM_ID;
...
const positionPercent = (raw: number): number => (raw / 65534) * 100;
...
const positionParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === POSITION_PARAM_ID);
assertEqual(positionParam?.name, 'CABINET_DYNACAB_R2', 'catalog param name');
assertEqual(positionParam?.unit, 'percent', 'catalog unit');
assertEqual(positionParam?.displayMin, 0, 'catalog displayMin');
assertEqual(positionParam?.displayMax, 100, 'catalog displayMax');
assertEqual(POSITION_INDEX, 200, 'channel-B Position bulk index');
...
try {
  ...
  const raw = values[POSITION_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_DYNACAB_R2 raw out of range: ${String(raw)}`);
  }

  const percent = positionPercent(raw);
  if (percent < -0.01 || percent > 100.01) {
    throw new Error(`Position percent out of range: ${percent.toFixed(2)}%`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_DYNACAB_R2 paramId=${POSITION_PARAM_ID} channel=B index=${POSITION_INDEX}`);
  console.log(`raw=${raw} catalogPercent=${percent.toFixed(2)}% values=${values.length}`);
} finally {
  conn.close();
}
