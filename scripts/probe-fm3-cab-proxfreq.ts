#!/usr/bin/env node
/**
 * Read-only FM3 CAB Proximity Frequency hardware validation.
 *
...
const PROXFREQ_PARAM_ID = 41;
const PROXFREQ_INDEX = CHANNEL * STRIDE + PROXFREQ_PARAM_ID;
...
const proxFreqParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === PROXFREQ_PARAM_ID);
assertEqual(proxFreqParam?.name, 'CABINET_PROXFREQ', 'catalog param name');
assertEqual(proxFreqParam?.unit, 'hz', 'catalog unit');
assertEqual(proxFreqParam?.displayMin, 20, 'catalog displayMin');
assertEqual(proxFreqParam?.displayMax, 200, 'catalog displayMax');
assertEqual(proxFreqParam?.scaling, 'log10', 'catalog scaling');
assertEqual(PROXFREQ_INDEX, 147, 'channel-B Proximity Frequency bulk index');
...
try {
  ...
  const raw = values[PROXFREQ_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_PROXFREQ raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_PROXFREQ paramId=${PROXFREQ_PARAM_ID} channel=B index=${PROXFREQ_INDEX}`);
