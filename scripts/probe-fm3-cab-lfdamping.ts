#!/usr/bin/env node
/**
 * Read-only FM3 CAB LF Damping hardware validation.
 *
...
const LFDAMPING_PARAM_ID = 57;
const LFDAMPING_INDEX = CHANNEL * STRIDE + LFDAMPING_PARAM_ID;
...
const lfDampingParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === LFDAMPING_PARAM_ID);
assertEqual(lfDampingParam?.name, 'CABINET_LFDAMPING', 'catalog param name');
assertEqual(lfDampingParam?.unit, 'numeric', 'catalog unit');
const lfDampingRange = FM3_RANGES.CABINET[LFDAMPING_PARAM_ID];
assertEqual(lfDampingRange?.kind, 'float', 'range kind');
assertEqual(lfDampingRange?.displayMin, 0, 'range displayMin');
assertEqual(lfDampingRange?.displayMax, 10, 'range displayMax');
assertEqual(LFDAMPING_INDEX, 163, 'channel-B LF Damping bulk index');
...
try {
  ...
  const raw = values[LFDAMPING_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_LFDAMPING raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_LFDAMPING paramId=${LFDAMPING_PARAM_ID} channel=B index=${LFDAMPING_INDEX}`);
