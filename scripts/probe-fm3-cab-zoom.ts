#!/usr/bin/env node
/**
 * Read-only FM3 CAB Zoom hardware validation.
 *
...
const ZOOM_PARAM_ID = 40;
const ZOOM_INDEX = CHANNEL * STRIDE + ZOOM_PARAM_ID;
...
const zoomParam = (FM3_PARAMS_BY_FAMILY.CABINET ?? []).find((p) => p.paramId === ZOOM_PARAM_ID);
assertEqual(zoomParam?.name, 'CABINET_ZOOM', 'catalog param name');
assertEqual(zoomParam?.unit, 'enum', 'catalog unit');
const zoomRange = FM3_RANGES.CABINET[ZOOM_PARAM_ID];
assertEqual(zoomRange?.kind, 'float', 'range kind');
assertEqual(zoomRange?.displayMin, 0, 'range displayMin');
assertEqual(zoomRange?.displayMax, 1, 'range displayMax');
assertEqual(ZOOM_INDEX, 146, 'channel-B Zoom bulk index');
...
try {
  ...
  const raw = values[ZOOM_INDEX];

  if (!Number.isInteger(raw) || raw < 0 || raw > 65534) {
    throw new Error(`channel-B CABINET_ZOOM raw out of range: ${String(raw)}`);
  }

  console.log(`PASS: ${detectedPath}`);
  console.log(`CABINET_ZOOM paramId=${ZOOM_PARAM_ID} channel=B index=${ZOOM_INDEX}`);
